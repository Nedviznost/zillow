...
export const useRedirectLoggedInUsers = () => {
  ...
  if (uid) {
    notify({ message: 'Логиран сте', type: 'warning' })
    router.push('/')
  }
}
...
export const useRedirectUnAuthenticatedUsers = () => {
  ...
  if (!uid) {
    notify({ message: 'Не сте логирани', type: 'warning' })
...
export const useAuthPageResponses = () => {
  ...
  useEffect(() => {
    if (error)
      notify({
        type: 'error',
        message: 'Неуспешна пријава. Ве молиме пробајте пак.',
      })
  }, [error])
...
export const useMyHomeEdit = (homeId: number, isEditPage: boolean) => {
  const router = useRouter()

  const { data } = useAppSelector(selectMoiDomovi)
  const isMyHome = data?.homes.filter((home) => home.id === homeId)
  useEffect(() => {
    if (!isMyHome && !!isEditPage) {
      notify({
        type: 'error',
        message: 'Ова не е ваш дом.',
      })
      router.push('/')
    }
  }, [isMyHome, router])
