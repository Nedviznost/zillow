...
import PopupMobileCard from '../PopupMobileContent'
...
export const Panel = ({ children, className, position }: MapPanelTypes) => {
  const classes = {
    ...
    'right-top': 'top-10 right-0 flex flex-col items-end text-right',
    'right-top-old': 'top-0 right-0 flex flex-col items-end text-right',
    'center-top':
  ...
  return (
    <div
      className={`absolute space-y-1 p-2 ${classes[position!]} ${className}`}
    >
      {children}
    </div>
  )
...
export const Error = () => {
  ...
  return error ? (
    <div className='px-4 py-2 text-white rounded-full shadow-xl bg-red'>
      Премногу пребарувања ве молиме почекајте.
    </div>
  ) : null
}
...
export const HomeMarkers = () => {
  // Факјанје дата Пребарување домовир на мапа
  // useFetchHomesMap()

  // const homes = useAppSelector(selectHomesMap).data?.homes || []
  const homes = useAppSelector(selectHomesActive)

  const dispatch = useAppDispatch()

  const markersTransitions = useTransition(homes || [], {
    keys: (home: any) => home.id,
  ...
  return markersTransitions((style: any, marker: any) => (
    <>
